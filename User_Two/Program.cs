...
namespace User_Two
{
    internal class Program
    {
        ...
        static async Task Main(string[] args)
        {
            ...
            httpClient.DefaultRequestHeaders.Add("User-Agent", $"User_Two");

            listener.Prefixes.Add("http://127.0.0.1:9587/");
        ...
        private static async Task Sender()
        {
            while (true)
            {
                ...
                var response = await httpClient.PostAsync("http://127.0.0.1:9586/", content);
                var response1 = await httpClient.PostAsync("http://127.0.0.1:9588/", content);
                var response2 = await httpClient.PostAsync("http://127.0.0.1:9589/", content);
